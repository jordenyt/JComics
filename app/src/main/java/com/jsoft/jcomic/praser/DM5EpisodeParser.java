...
public class DM5EpisodeParser extends EpisodeParser {
    ...
    protected void getEpisodeFromUrlResult(List<String> result) {
        ...
        while (m.find()) {
            ...
            String s1= m.group(1);
            Pattern p1 = Pattern.compile("\'([a-zA-Z0-9_|]*?)\'\\.split");
            ...
            while (m1.find()) {
                ...
                for (int j=0; j < replaceStrArray.length; j++) {
                    //Log.e("jComics 1", replaceStrArray[j]);
                }
            }
            String s2= m.group(1);
            Pattern p2 = Pattern.compile("[0-9a-z] [0-9a-z]=\\[(.+?)\\]");
            ...
            while (m2.find()) {
                ...
                for (int i=0; i<urls.length; i++) {
                    //Log.e("jComics 2", urls[i]);
                    String imgUrl = urls[i];
                    imgUrl = imgUrl.replaceAll("\\\\\\'", "");
                    for (int j=0; j < replaceStrArray.length; j++) {
                        if (replaceStrArray[j].length() > 0) {
                            imgUrl = imgUrl.replaceAll("\\b" + characters.charAt(j) + "\\b", replaceStrArray[j]);
                        }
                    }
                    imageUrlList.add(imgUrl);
                    //Log.e("jComics 2", imgUrl);
                }
            }
        }
